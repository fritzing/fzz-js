...
var _require = require('fzp-js'),
    FZP = _require.FZP,
    FZPUtils = _require.FZPUtils;

var FZBoard = require('./board');

var _require2 = require('./connector'),
    FZConnector = _require2.FZConnector;

var _require3 = require('./instance'),
    FZInstance = _require3.FZInstance,
    FZInstanceView = _require3.FZInstanceView;

var FritzingAPI = 'https://fritzing.github.io/fritzing-parts';
...
function createFZPsMap(xml) {
  ...
  for (var i = 0; i < xml.length; i++) {
    for (var j = 0; j < xml[i].instance.length; j++) {
      var modId = xml[i].instance[j].$.moduleIdRef;
      var path = xml[i].instance[j].$.path;
...
function parseInstanceView(xml) {
  var instanceView = new FZInstanceView({
    layer: xml[0].$.layer,
    bottom: xml[0].$.bottom,
  ...
  if (xml) {
    if (xml[0]) {
      for (var i = 0; i < xml[0].connect.length; i++) {
        var id = xml[0].connect[i].$.connectorId;
        connects[id] = {
...
function parseFZViews(xml) {
  ...
  for (var j = 0; j < xml[0].view.length; j++) {
...
module.exports = { FZ: FZ, parseFZ: parseFZ };